...
@propertyWrapper
public struct OptionalLosslessValue<T: LosslessStringCodable>: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        do {
            self.wrappedValue = try Optional<T>.init(from: decoder)
            self.type = T.self
        } catch let error {
            ...
            guard let rawValue = types.lazy.compactMap({ $0(decoder) }).first, let value = T.init("\(rawValue)") else {
                throw error
            }
...
extension OptionalLosslessValue: Equatable where T: Equatable {
    public static func == (lhs: OptionalLosslessValue<T>, rhs: OptionalLosslessValue<T>) -> Bool {
        return lhs.wrappedValue == rhs.wrappedValue
    }
}

extension OptionalLosslessValue: Hashable where T: Hashable {
